...
void Stack_Init( Stack *stack ) {
	if (stack == NULL) {	// overenie existencie stacku
		Stack_Error(SERR_INIT);		
	} else {
		stack->topIndex = -1;	// inicializacia na -1
	}
}
...
int Stack_IsFull( const Stack *stack ) {
	// STACK_SIZE-1 kvoli indexacii, ak mam stack_size==3, tak mozem indexovat do max topIndex==2 
...
void Stack_Top( const Stack *stack, char *dataPtr ) {
	if (Stack_IsEmpty( stack )) {	// overenie ci stack nie je prazdny
		Stack_Error(SERR_TOP);
	} else {
		*dataPtr = stack->array[stack->topIndex];	// priradenie dat do pointra
	}
}
...
void Stack_Pop( Stack *stack ) {
	...
	if (Stack_IsEmpty( stack )) { // overenie
		return;
	} else {
		stack->topIndex--; // dekrement indexu
		return;
	}
...
void Stack_Push( Stack *stack, char data ) {
	if (Stack_IsFull( stack )) {	// overenie plneho stacku
		Stack_Error( SERR_PUSH );
	} else {
		stack->topIndex++;	// inkrement indexu
		stack->array[stack->topIndex] = data; // ulozenie dat
	}
}
